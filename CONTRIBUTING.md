...
- 機能追加やバグ修正を始める前に、**必ず Issue を作成**してください。
- ラベルで分類：`bug`、`feat`、`docs`、`breaking`
- 作業開始時に自分をアサインしてください。
...
- PR タイトルは [Conventional Commits](https://www.conventionalcommits.org/) に従う（例：`feat: 新しい入出金CSVフォーマットに対応`）
- PR 本文で Issue を参照：`Closes #123`
- 破壊的変更の場合、PR 本文に `BREAKING CHANGE:` と移行手順を記載
- バージョンバンプのヒントとしてラベルを付与：`patch`、`minor`、`major`

### バージョンバンプ

- **コントリビューターはバージョンを変更しないでください。** メンテナーがリリース時に PR ラベルと変更履歴に基づいてバージョンを決定します。
- CI はすべての PR でリント＋テストを実行します。マージにはすべてのチェックが通る必要があります。
...
このプロジェクトは [Semantic Versioning](https://semver.org/) に従います：`MAJOR.MINOR.PATCH`

| バンプ | 条件 | 例 |
|--------|------|-----|
| **PATCH** (0.1.2 → 0.1.3) | バグ修正、docstring 更新、CLI 変更なしの内部リファクタ | HTML パースのエッジケース修正 |
| **MINOR** (0.1.3 → 0.2.0) | 後方互換性のある新機能（新フォーマット対応、新オプション） | 新しい入出金 CSV フォーマットに対応 |
| **MAJOR** (0.2.0 → 1.0.0) | 破壊的変更（コマンド削除/名前変更、設定フォーマット変更） | パーサー API の変更 |

> pre-1.0（`0.x.y`）の間は、マイナーバンプに小さな破壊的変更が含まれる場合があります。1.0 以降は厳密な SemVer を適用します。
